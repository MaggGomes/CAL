...
public:
	...
	void setDpProcessing(bool dp){
		this->dpProcessing = dp;
	}
	bool getDpProcessing() const {
		return dpProcessing;
	}
...
template <class T>
Vertex<T>::Vertex(T in): info(in), visited(false), processing(false), dpProcessing(false), indegree(0), dist(0) {
	path = NULL;
}
...
vector<int> Graph<T>::testPoints(int start,int end,vector<int> toPass, vector<int> path,vector<bool>vecB){
	//cout << start << " " << end << " " << toPass.size() <<endl;
